                ...
                <table class="w-full text-sm text-left min-w-[800px]">
                    <thead class="bg-gray-100 text-gray-600 text-center">
                        <tr>
                            ...
                            <th class="px-4 py-3">Tanggal STNK</th>
                    ...
                    <tbody class="divide-y text-center">
                        @foreach ($orders as $order)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">{{ $order->stnk_date }}</td>
                                ...
                                <td class="px-4 py-3">
                                    <form method="POST" action="{{ route('admin.orders.updateStatus', $order->id) }}">
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            class="text-xs text-center px-2 py-1 rounded-full
                                                @if ($order->status == 'dalam proses') bg-yellow-100 text-[#0A3D62]
                                                @elseif ($order->status == 'pesanan diterima') bg-[rgba(7,222,255,0.4)] text-[#0A3D62]
                                                @elseif ($order->status == 'siap diambil') bg-green-100 text-[#0A3D62]
                                                @elseif ($order->status == 'selesai') bg-red-100 text-[#0A3D62]
            ...
            <div class="md:hidden space-y-4">
                @foreach ($orders as $order)
                    <div class="bg-white rounded-xl shadow-md p-4">
                        ...
                        <p class="text-sm text-gray-700"><strong>TanggalSTNK:</strong> {{ $order->stnk_date }}</p>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-12 text-sm">
                            <template
                                ...
                                'Tanggal Berlaku STNK': new Date(selectedOrder.stnk_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }),
                ...
                <form :action="'/admin/orders/' + selectedOrder.id + '/update'" method="POST"
                    ...
                    <div class="mb-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="phone" :value="selectedOrder.phone"
                                    class="w-full border rounded p-2 bg-white">
                            </div>
                            <div>
                    ...
                    <div class="mb-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block font-semibold text-[#0A3D62]">Masa Berlaku STNK</label>
                                <input type="date" name="stnk_date" :value="selectedOrder.stnk_date"
                                    class="w-full border rounded p-2 bg-white">
                            </div>
                            <div>
